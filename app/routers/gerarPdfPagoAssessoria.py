...
def preparar_htmlPago(dados, numero_demonstrativo, tipo: str = "assessoria", dados_assessoria: dict | None = None):
    """
    ...
    for dia, lista in list(dados_por_dia.items()):
        ...
        for item in lista:
            ...
            if not isinstance(item, dict):
                try:
                    ...
                    item = {
                        ...
                        "percentual_assessoria": float(getattr(item, "percentual_assessoria", 0) or 0),
                        "valor_assessoria": float(getattr(item, "valor_assessoria", 0) or 0),
                    }
                except Exception:
                    item = {}
            ...
            safe = {
                ...
                "percentual_assessoria": float(item.get("percentual_assessoria", 0) or 0),
                # nome da chave usada no template final: valor_assessoria
                "valor_assessoria": float(item.get("valor_assessoria", item.get("comissao_valor", 0) or 0)),
            }
